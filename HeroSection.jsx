...
export const HeroSection = () => {
  ...
  const codeSnippets = [
    "import { JuniorDeveloper } from 'talha.dev';",
    "",
    "const developer = new JuniorDeveloper({",
    "  name: 'Talha Asif',",
    "  stack: ['React', 'PHP', 'MySQL', 'JavaScript'],",
    "  focus: 'Building responsive web apps',",
    "  status: 'Open to new opportunities'",
    "});",
    "",
    "await developer.launchPortfolio();",
    "// Auth System, Event Booking, E-Commerce",
    "",
    "developer.connect();",
    "console.log('Let us build something great!');"
  ];

  const achievements = [
    { number: "1+", label: "Years Experience", icon: <Shield className="h-3 w-3" /> },
    { number: "3+", label: "Projects Delivered", icon: <TrendingUp className="h-3 w-3" /> },
    { number: "100%", label: "Client Satisfaction", icon: <Award className="h-3 w-3" /> },
    { number: "3.0", label: "GPA Score", icon: <Zap className="h-3 w-3" /> }
  ];
  ...
  const handleViewResume = () => {
    window.open('/Talha-resume.pdf', '_blank', 'noopener,noreferrer');
  };
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8 overflow-hidden bg-gradient-to-br from-background via-background/95 to-primary/10" ref={ref}>
      <div className="absolute inset-0 -z-10 overflow-hidden">
        ...
        <motion.div className="absolute top-20 left-10 w-72 h-72 rounded-full bg-gradient-to-r from-primary/10 to-purple-600/10 blur-[100px]" animate={{ x: [0,30,0], y: [0,-30,0] }} transition={{ duration: 15, repeat: Infinity }} />
        <motion.div className="absolute bottom-20 right-10 w-72 h-72 rounded-full bg-gradient-to-r from-cyan-400/10 to-emerald-500/10 blur-[100px]" animate={{ x: [0,-40,0], y: [0,40,0] }} transition={{ duration: 20, repeat: Infinity, delay: 2 }} />
      </div>
        ...
        >
          <div className="flex-1 text-center lg:text-left max-w-2xl mx-auto lg:mx-0">
            ...
            <motion.h1
              ...
              variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}
            >
              <span className="block text-foreground">I'm Talha Asif</span>
              <motion.span
            ...
            <motion.p
              ...
              variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}
            >
              I build <span className="text-primary font-semibold">responsive, scalable web applications</span> using React.js, PHP REST APIs, and MySQL. Currently working at The Websters, Mian Channu.
            </motion.p>
            ...
            >
              {achievements.map((a, i) => (
                <div key={i} className="text-center p-4 rounded-xl bg-background/60 border border-border/50 backdrop-blur-sm hover:border-primary/30 transition-all duration-300">
                  <div className="flex items-center justify-center gap-2 mb-2">
                    {a.icon}
                    <div className="text-2xl font-bold text-foreground">{a.number}</div>
                  </div>
            ...
            <motion.div
              ...
              variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}
            >
              <motion.a href="#projects" className="group px-8 py-4 rounded-xl font-semibold bg-gradient-to-r from-primary to-purple-600 text-primary-foreground shadow-lg text-sm flex items-center justify-center gap-3" whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                <Code className="h-5 w-5" />
                <span>View My Projects</span>
                <TrendingUp className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
              </motion.a>
              <motion.a href="#contact" className="group px-8 py-4 rounded-xl font-semibold border border-primary/50 text-foreground hover:border-primary transition-all duration-300 bg-background/80 backdrop-blur-sm text-sm flex items-center justify-center gap-3" whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                <Mail className="h-4 w-4" />
                <span>Contact Me</span>
              </motion.a>
              <motion.button onClick={handleViewResume} className="group px-6 py-4 rounded-xl font-semibold border border-border text-muted-foreground hover:border-primary/30 transition-all duration-300 bg-background/60 backdrop-blur-sm text-sm flex items-center justify-center gap-2" whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
            ...
            >
              <div className="text-sm text-muted-foreground">
                🚀 <span className="text-primary font-semibold">Available Immediately</span> for Frontend and Full-Stack roles
              </div>
            </motion.div>
          </div>
          ...
          >
            <div className="relative w-full max-w-md">
              <motion.div className="bg-background/90 border border-border rounded-2xl p-8 backdrop-blur-sm shadow-2xl w-full" whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 400, damping: 25 }}>
                ...
                <div className="font-mono text-sm bg-primary/5 rounded-lg border border-primary/10 min-h-[280px] p-6">
                  {codeSnippets.map((line, index) => (
                    <div
                      key={index}
                      className={`min-h-[20px] ${
                        index < currentCodeLine ? 'opacity-100' : index === currentCodeLine ? 'opacity-100' : 'opacity-0'
                        ...
                        : line.includes("React") || line.includes("PHP") || line.includes("MySQL") ? "text-cyan-400"
                ...
                <motion.div className="absolute -bottom-3 -right-3 w-14 h-14 bg-gradient-to-r from-primary to-purple-600 rounded-xl flex items-center justify-center border-2 border-background shadow-2xl" animate={{ y: [0,-5,0] }} transition={{ duration: 4, repeat: Infinity }}>
                  <Code className="h-5 w-5 text-white" />
                </motion.div>
                <motion.div className="absolute -top-3 -left-3 bg-background/90 backdrop-blur-sm px-4 py-2 rounded-xl border border-border shadow-lg flex items-center gap-2" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 1.5, type: "spring" }}>
                ...
                <motion.div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 bg-background/90 backdrop-blur-sm px-4 py-2 rounded-xl border border-border shadow-lg text-center" initial={{ scale: 0, y: 20 }} animate={{ scale: 1, y: 0 }} transition={{ delay: 2, type: "spring" }}>
                  ...
                  <div className="text-sm font-bold text-foreground">React & PHP</div>
                </motion.div>
              </motion.div>
            </div>
          </motion.div>
      ...
      <motion.div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex flex-col items-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: [0,1,1,0], y: [0,6,0,-6] }} transition={{ duration: 3, repeat: Infinity, repeatDelay: 0.5 }}>
        <motion.div className="text-xs text-primary mb-3 flex items-center gap-2 px-4 py-2 rounded-full bg-background/80 backdrop-blur-sm border border-border shadow-lg">
